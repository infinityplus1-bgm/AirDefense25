...
class LaserPowerController(QWidget):
    ...
    def update_current_power_display(self, is_system_active: bool):
        ...
        if is_system_active: # Simulate adjustment towards target
            diff = target_val - self.current_power
            if abs(diff) > 2:
                adjustment = int(diff / 5) or (1 if diff > 0 else -1)
                self.current_power += adjustment
            else: # Close to target, add small fluctuation
                self.current_power += random.randint(-1, 1)
        else: # Simulate power decay or low fluctuation if inactive
            self.current_power = max(0, self.current_power - random.randint(1, 3)) + random.randint(-1, 1)

        self.current_power = max(0, min(100, self.current_power)) # Clamp 0-100

        self.current_power_display_label.setText(f"{self.current_power}%")
        self.current_power_display_label.setStyleSheet(f"color: {self._get_power_color(self.current_power)};")
